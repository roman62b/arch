...
bool menu(void);
uint8_t inp_usr_step(void);
uint8_t calc_enemy_step(char cur_field[]);
...
void mk_usr_step() {
    again: ;
    uint8_t step_val = inp_usr_step() - 1;
...
uint8_t calc_enemy_step(char cur_field[]) {
    // attack
    for (uint8_t cell = 0; cell < FIELD_SIZE; ++cell) {
        if (cur_field[cell] == ' ') {
    ...
    uint8_t *free_cells = malloc(free_size * sizeof(uint8_t));
    if (free_cells == NULL) exit(1);
    uint8_t index = 0;
    for (uint8_t cell = 0; cell < FIELD_SIZE; ++cell) {
        if (cur_field[cell] == ' ') {
    ...
    srand(time(NULL));
    uint8_t random_cell = free_cells[(rand() % free_size)];
    free(free_cells);
    return random_cell;
}
...
void clear_data() {
    is_win = false;
    is_drawn = false;
    for (uint8_t i = 0; i < FIELD_SIZE; ++i) { field[i] = ' '; }
}
...
bool get_stats(const char *file_name) {
    ...
    if (file) {
        ...
        for (uint8_t i = 0; i < STATS_COUNT; ++i) { stats[i] = 0; }
        return true;
    }
}
